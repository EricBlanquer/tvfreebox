...
function checkOptions() {
	...
	xhr.open('GET', 'http://hd' + boitier + '.freebox.fr/pub/remote_control?code=' + code + '&key=' + key + '&a=' + Math.random(), true);
...
function newConfig(firstConfig) {
	...
	configs[configs.length] = { label: label, version: version, boitier: boitier, code: code, zoom: zoom, help: help, inpopup: inpopup, alphakeys: alphakeys };
	var cptLabel = 1;
	if(!firstConfig) {
		var found;
		do {
